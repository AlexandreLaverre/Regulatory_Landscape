...
output = open("../data/"+sp+"/simulations_5Mb_bin5kb_alloverlap.txt", 'w')
if os.stat("../data/"+sp+"/simulations_5Mb_bin5kb_alloverlap.txt").st_size == 0:
...
for frag in dist.keys():
    ...
    nb = 0
    for x in simul:
        if x not in dic_test.keys():
            dic_test[x] = proba_bis[nb]
        else:
            dic_test[x] = dic_test[x]+proba_bis[nb]

        nb += 1

    fragments = list(dic_test.keys())
    proba = list(dic_test.values())

    # Selection of same number of unique restriction frag as in observed data
    select = np.random.choice(fragments, len(dist[frag]), p=proba, replace=False)
