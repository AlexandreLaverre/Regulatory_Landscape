...
obs <- read.table(paste(path, "Supplementary_dataset3_annotations/", ref_sp, "/contacted_sequence_composition_", ref_sp, "_observed.txt", sep=""), header=T)
simul <- read.table(paste(path, "Supplementary_dataset3_annotations/", ref_sp,"/contacted_sequence_composition_", ref_sp, "_simulated.txt", sep=""), header=T)
data_list <- list(obs, simul)
data_names <- c("Original", "Simulated")
obs <- obs[which(obs$baited == "unbaited" & obs$duplication == 0),]
simul <- simul[which(simul$baited == "unbaited" & simul$duplication == 0),]

data_list <- lapply(data_list, function(data) data[which(data$baited == "unbaited" & data$duplication == 0),])
...
obs$dist_class <-cut(obs$midist, breaks=seq(from=25000, to=5000000, by=50000), include.lowest = T)
simul$dist_class <- cut(simul$midist, breaks=seq(from=25000, to=5000000, by=50000), include.lowest = T)
...
save(enh_prop, prop_nb_sample, prop_dist, file = paste(path, "/Main_figures/Fig2_human.Rdata", sep=""))


# Proportion of the sequences that is repeat eleme or phastcons
