...
for(ref_sp in c("human", "mouse")){
  ...
  pvals_coverage[[ref_sp]] <- c()
   for(enh in enhancers){
    ...
    test =chisq.test(matrix(c(overlap, non.overlap), nrow=2))
    print(test)
    
    if (test$p.value == 0){pval=2.2e-16}else{pval=test$p.value}
    pvals_coverage[[ref_sp]] <- c(pvals_coverage[[ref_sp]], pval)
