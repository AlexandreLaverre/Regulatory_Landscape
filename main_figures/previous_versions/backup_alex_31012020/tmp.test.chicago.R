...
if(load){
  ...
  for(sp in c("human", "mouse")){
    ...
    for (enh in enhancer.datasets[[sp]]){
      enh_frag = read.table(paste0("/home/laverre/Manuscript/SupplementaryDataset4/human/", enh, "/overlap_restriction_fragments_enhancers.txt"), h=T)
      
      variants = c("chr6:27818878:27821827:+","chr6:27822024:27822736:+","chr6:27822737:27823187:+","chr6:27833514:27834330:+","chr6:27869843:27872603",
                   "chr7:25845271:25848220:+","chr7:25848221:25852797:+","chr7:25852798:25854577:+","chr7:25854578:25864445:+","chr7:25864446:25869574:+",
                   "chr7:25869575:25872596:+","chr7:25872597:25874256:+", 
                   "chr2:177146808:177149154:+", "chr10:99429975:99430708:+")
      
      enh_frag = enh_frag[which(enh_frag$ID %in% variants),]
      enh_in_var = unlist(strsplit(enh_frag$overlap_ID, split=',', fixed=TRUE))
      ...
      contact_obs = contact_obs[which(contact_obs$origin_enh %notin% enh_in_var),]
...
frag = "chr7:25845271:25848220"
stat_frag = obs[which(obs$id_frag == frag),]
nrow(stat_frag)
mean(stat_frag$dist)
mean(stat_frag$median_score)

test = obs

nb_bait <- tapply(test$id_bait, as.factor(test$id_frag), function(x) length(x))
mean_score <- tapply(test$median_score, as.factor(test$id_frag), function(x) mean(x))
med_dist <- tapply(test$dist, as.factor(test$id_frag), function(x) median(x))

test.stats = data.frame("nb_bait"=nb_bait, "mean_score"=mean_score, "med_dist"=med_dist)
test.stats[which(test.stats$nb_bait>15 & test.stats$mean_score>10 & test.stats$med_dist>1000000),]


test.stats$class_dist <- cut(test.stats$start, breaks=seq(from=0, to=max(test.stats$start), by=30000), include.lowest = T)

plot.by.class <- tapply(test.stats$med_score, as.factor(test.stats$class_dist), function(x) mean(x, na.rm=T))
plot(plot.by.class, xlab="", ylab="", axes=F, ylim=c(1, 30))
axis(side=2, mgp=c(3, 0.75, 0))
mtext("Nb baits", side=2, line=2.5)
for (i in 1:length(plot.chic.frag_high)){segments(x0=i, y0=plot.chic.frag_low[i], x1=i, y1=plot.chic.frag_high[i])}

xax=c(1, 10,  20, 30, 40)
axlab=as.character(c(0.05,  0.5,  1, 1.5, 2))
axis(side=1, mgp=c(3, 0.65, 0), at=xax, labels=axlab)
mtext("distance to promoters (Mb)", side=1, line=2)




test.stats = test.stats[order(test.stats$start),]
plot(test.stats$nb_bait)
