...
CEX = 1.8
CEX_lines = 1.3
png("Fig4.png", width = 800, height = 800)
par(mfrow=c(4,1), mai = c(0.5, 0.7, 0.5, 0.2))

layout(matrix(c(1,1,2,2,3,4,5,6), 4, 2, byrow = TRUE))

####### A - Large scale conservation #######
plot(obs_conserv$score, type="b", col="red", cex=CEX_lines, main=paste("Conservation by number of cell types", sep=""), ylim=c(0,0.8),
...
plot(obs_phastcons$inter[1:50], type="l", col="red", cex=CEX_lines,main=paste("PhastCons non-exonic composition", sep=""),
     xlab="", ylab="Phastcons proportion", xaxt = "n", ylim=c(0.03,0.08), cex.lab=CEX, cex.axis=CEX, cex.main=CEX)
