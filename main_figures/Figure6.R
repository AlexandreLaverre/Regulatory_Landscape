...
plot.expdiv.regdiv <- function(regland, feature, expdata, distances, ylab, plot.label, xlab, xax.labels){
  ...
  mtext(plot.label, side=3, line=1.5, at=plot.lab.pos, font=2, cex=1.2)
  
  axis(side=1, cex.axis=1, mgp=c(3, 0.75, 0), at=xpos, labels=rep("",length(xpos)))
  mtext(xax.labels, at=xpos, side=1, line=0.75, cex=0.75)
