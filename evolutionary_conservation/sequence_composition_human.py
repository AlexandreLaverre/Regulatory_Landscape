...
def composition_seq(origin_sp, data):
        ...
        for i in f3.readlines():
            ...
            if i[0] == i[3]:
                ...
                if 25000 <= dist_obs <= 10000000:
                    ...
                    RoadMap_contact[bait] += RoadMap_count[merged_PIR]
                    GRO_seq_contact[bait] += GRO_seq_count[merged_PIR]


    print("Writting output...")
    output = open("../../result/conservation/contacted_sequence_composition_" + origin_sp + data + merge + ".txt", 'w')
    if os.stat(
            "../../result/conservation/contacted_sequence_composition_" + origin_sp + data + merge + ".txt").st_size == 0:
        output.write("chr\tstart\tend\tCAGE_count\tENCODE_count\tRoadMap_count\tGRO_seq_count\tbait_contacted\tbait_complexity"
                     "\tmidist_obs\tbaited\tnb_cell\tduplication\tall_exon_pb\tall_exon250\tcoding_exon_pb"
                     "\tnocoding_exon_pb\trepeat_pb\tphastcons_noexonic250\tTSS_count\t" + '\t'.join(cell_name) + "\n")
    ...
    for PIR in inter.keys():
        ...
        output.write(PIR.split(':')[0] + '\t' + PIR.split(':')[1] + '\t' + PIR.split(':')[2]
                     + '\t' + str(CAGE_count[PIR]) + '\t' + str(ENCODE_count[PIR]) + '\t' + str(RoadMap_count[PIR])
                     + '\t' + str(GRO_seq_count[PIR]) + '\t' + str(len(inter[PIR])) + '\t' + str(np.mean(bait_contact))
                     + '\t' + str(np.median(inter[PIR])) + '\t' + str(PIR_baited[PIR]) + '\t' + str(len(PIR_nbcell[PIR]))
    ...
    if os.stat("../../result/conservation/bait_composition_" + origin_sp + data + merge + ".txt").st_size == 0:
        output_bait.write("chr\tstart\tend\tCAGE_contacted\tENCODE_contacted\tRoadMap_contacted\tGRO_seq_contacted"
                          "\tPIR_contacted\tunbaited_PIR_contacted\tPIR_complexity\tmidist_obs\tduplication\tall_exon_pb"
                          "\tcoding_exon_pb\tnocoding_exon_pb\trepeat_pb\tphastcons_noexonic250\tTSS_count1Kb\tgenes_count1Kb\n")
    ...
    for Bait in inter_bait.keys():
        ...
        output_bait.write(Bait.split(':')[0] + '\t' + Bait.split(':')[1] + '\t' + Bait.split(':')[2]
                          ...
                          + '\t' + str(RoadMap_contact[Bait]) + '\t' + str(RoadMap_contact[Bait])
