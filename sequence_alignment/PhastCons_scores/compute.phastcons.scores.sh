...
if [ ${cluster} = "in2p3" ]; then
    export path=/sps/biometr/necsulea/LncEvoDevo
fi

if [ ${cluster} = "pbil" ]; then
    export path=/beegfs/data/necsulea/LncEvoDevo
fi
...
do
	...
	if [ -e ${pathResults}/PhastCons_Promoters_${size}_chr${chr}_${suffixProm}.txt ]; then
	    ...
	    echo "perl ${pathScripts}/compute.phastcons.scores.pl --pathCoords=${pathPromoters}/PromoterCoords_${size}_${suffixProm}.bed  --pathMaskExonBlocks=${pathExons}/${suffixActualExons}.txt --pathPhastCons=${pathPhast} --chr=chr${chr} --pathOutput=${pathResults}/PhastCons_Promoters_MaskedExons_${size}_chr${chr}_${suffixProm}.txt" >> bsub_script
