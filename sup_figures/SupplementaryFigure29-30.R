...
pdf(paste(pathFigures, "/SupplementaryFigure30_bis.pdf", sep=""), width=7, height=7)
par(mfrow=c(3,2))
...
CellTypesPlot <- function(var, plot.nb){
  if (var == "Conservation"){ylim=c(0.2, 0.4)}
...
pdf(paste(pathFigures, "/SupplementaryFigure29_bis.pdf", sep=""), width=7, height=7)
