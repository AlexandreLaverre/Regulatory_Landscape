...
for(sp in c("human", "mouse")){
  for(enh in c("ENCODE", "FANTOM5")){
    ...
    if(length(obs[[sp]][[enh]])>0){
      ...
      ymin=min(c(obs[[sp]][[paste0(enh,"_conflow")]], obs[[sp]][[paste0(enh,"_confup")]], neighbors[[sp]][[paste0(enh,"_conflow")]], neighbors[[sp]][[paste0(enh,"_confup")]]), na.rm=T)
      ymax=max(c(obs[[sp]][[paste0(enh,"_conflow")]], obs[[sp]][[paste0(enh,"_confup")]], neighbors[[sp]][[paste0(enh,"_conflow")]], neighbors[[sp]][[paste0(enh,"_confup")]]), na.rm=T)
      ...
      class_leg <- c("0.05", "0.25", "0.5", "0.75", "1")
      axis(side=1, at=c(0.5, 9.5, 19.5, 29.5, 39.5), labels=class_leg, mgp=c(3, 0.65, 0), cex.axis=0.95)
      axis(side=2, mgp=c(3, 0.65, 0), cex.axis=0.95, las=2)
      
      mtext("Spearman's rho", side=2, cex=0.75, line=2.5, at=(ymin+ymax)*0.9/2)
      mtext("distance to promoters (Mb)", side=1, line=2, cex=0.75)
      mtext(paste(sp, enh), side=3, line=0.5, cex=0.8)

      mtext(labels[index], side=3, line=0.5, font=2, at=-11.5)

      if(index==1){
        legend("bottomleft", bty="n", col=c("darkorange", "black"), legend=c("contacts", "neighbors"), inset=0.01, pch=20)
      }
