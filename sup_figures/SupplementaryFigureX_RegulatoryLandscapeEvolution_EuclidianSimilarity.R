...
DivergenceMeasure = c("EuclidianSimilarity", "ResidualEuclidianSimilarity") # "EuclidianSimilarity" or "CorrelationSpearman" or SpearmanResidual

pdfname="SupplementaryFigureX_RegulatoryLandscapeEvolution_EuclidianSimilarity.pdf"

################################################################################################################################
#pdf(file=paste(pathFigures, pdfname, sep=""), height=5)

for (DivergenceMeasure in DivergenceMeasures){
  ##############################################################################################################################
  ############################## PART1 : All cells & Cardoso-Moreira  ##########################################################
  if (DivergenceMeasure == "EuclidianSimilarity"){ylim=c(-0.03, 0.1)}else{ylim=c(-0.03, 0.1)}
  ...
  mtext(DivergenceMeasure, side=2, line=2.5, cex=0.9)
