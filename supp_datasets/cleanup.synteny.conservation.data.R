  ...
  for(tg in setdiff(allspecies, ref)){
    for(enh in enhancer.datasets[[ref]]){
      for(data in c("original", "simulated")){
        data=read.table(paste(pathEvolution, ref, "/synteny_conservation/",enh,"/",ref,"2",tg,"_",data,"_synteny.txt",sep=""), h=T, stringsAsFactors=F)
        data=data[,which(!(colnames(data)%in%exclude))]
        
        system(paste("mv ", pathEvolution, ref, "/synteny_conservation/",enh,"/",ref,"2",tg,"_",data,"_synteny.txt ",pathEvolution, ref, "/synteny_conservation/",enh,"/backup_",ref,"2",tg,"_",data,"_synteny.txt ", sep=""))
        
        write.table(data, file=paste(pathEvolution, ref, "/synteny_conservation/",enh,"/",ref,"2",tg,"_",data,"_synteny.txt",sep=""), row.names=F, col.names=T, sep="\t", quote=F)
      }
    }
  }
}
