  ...
  for(tg in setdiff(allspecies, ref)){
    for(enh in enhancer.datasets[[ref]]){
      for(type in c("original", "simulated")){
        ...
        system(paste("mv ", pathEvolution, ref, "/synteny_conservation/",enh,"/",ref,"2",tg,"_",type,"_synteny.txt ",pathEvolution, ref, "/synteny_conservation/",enh,"/backup_",ref,"2",tg,"_",type,"_synteny.txt ", sep=""))
