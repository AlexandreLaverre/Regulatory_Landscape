...
sub extractMappedRegions{
	...
	if(exists $coords->{$id}){
		...
		if($thisstart>=$currentstart && $thisstart<=($currentend+1)){
		    ...
		    if(exists $regions->{$thischr}){
			push(@{$regions->{$thischr}{"start"}},$currentstart);
			push(@{$regions->{$thischr}{"end"}},$currentend);
		    }
		    else{
			$regions->{$thischr}={"start"=>[$currentstart],"end"=>[$currentend]};
		    }
...
my @defaultpars=("pathReads","pathAlignment","readLength","chr", "pathOutput");
my %defaultvalues;
...
checkAlignments($parameters{"pathAlignment"}, $chr, \%coordinates, $readlen);
...
my %mapped;
extractMappedRegions(\%coordinates, $chr, $nbtotreads, $readlen, \%mapped);
print "Done.\n";
