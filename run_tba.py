...
sp1_genome = path+sp1+"_restriction_fragments.fa"
...
with open(path+file, 'r') as f1:
	...
	i = i.split("\t")
        ...
        os.mkdir(path+'sp1_frag_%s/' % i[0])
	os.chdir(path+'sp1_frag_%s/' % i[0])
	    ...
	    os.system("mv tba.maf.gz "+path+"tba_alignments/"+sp1+"2"+sp2+"/"+i[0]+"."+i[1]+".maf.gz")
	    os.system("rm -r "+path+"sp1_frag_"+i[0])
