...
plot(conserv_dist$inter[1:50], type="b", col="red",  ylim=c(0.15,0.28), main="Mouse2Human", xlab="",ylab="Mean score conserv", xaxt = "n")
...
text(seq(1,51,10), par("usr")[3]-0.006, labels = class_leg, srt = 45, pos = 1, xpd = TRUE, cex=0.8)
...
plot(conserv_dist$inter[1:50], type="b", col="red", ylim=c(42,70), main="Mouse2Human", ylab="Proportion fragment conserv (%)", xlab="", xaxt = "n")
for (row in 1:nrow(conserv_dist[1:50,])){
  segments(x0=row,y0=conserv_dist[row,]$int_start,x1=row,y1=conserv_dist[row,]$int_end, col='red', lwd=0.3)}
points(simul_dist$inter[1:50], type="b", col="blue")
for (row in 1:nrow(simul_dist[1:50,])){
  segments(x0=row,y0=simul_dist[row,]$int_start,x1=row,y1=simul_dist[row,]$int_end, col='blue', lwd=0.3)}
axis(1, at=seq(1,51,10), labels=F)
text(seq(1,51,10), par("usr")[3] - 1, labels = class_leg, srt = 45, pos = 1, xpd = TRUE, cex=0.8)
