...
export enhancer=$3        # i.e : FANTOM5 ENCODE RoadmapEpigenomics FOCS_GRO_seq contacted_sequence_original contacted_sequence_simulated
...
export pathEnhancers=${pathAnouk}/SupplementaryDataset5/${species}/${enhancer}
...
do
    ...
    if [ -e "${pathPhyloP_scores}" ]; then
      if [ -e "${pathResults}"/phyloP_${way}_chr${chr}_${suffixExons}.txt ]; then
	    ...
	    echo "perl ${pathScripts}/compute.phyloP.scores.pl --pathCoords=${pathEnhancers}/enhancer_coordinates.bed   --pathMaskExonBlocks=${pathExons} --pathPhastCons=${pathPhyloP_scores} --chr=chr${chr} --pathOutput=${pathResults}/phyloP_${way}_chr${chr}_${suffixExons}.txt" >> ${pathScripts}/log/phyloP_${way}_${species}_${enhancer}_chr${chr}_${suffixExons}
	    #--pathCoords=${pathEnhancers}/statistics_${enhancer}.txt

      #bash ${pathScripts}/log/phyloP_${way}_${species}_${enhancer}_chr${chr}_${suffixExons}
      sbatch -p normal --time=1:00:00 --mem=20GB -c 1 -o ${pathScripts}/log/std_output_phyloP_${way}_${species}_${enhancer}_chr${chr}_${suffixExons} -e ${pathScripts}/log/std_error_phyloP_${way}_${species}_${enhancer}_chr${chr}_${suffixExons} ${pathScripts}/log/phyloP_${way}_${species}_${enhancer}_chr${chr}_${suffixExons}
       fi
    fi
done
